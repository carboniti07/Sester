...
Format: [Keep a Changelog](https://keepachangelog.com/) Â· SemVer.
...
- **Transaction-signing interface (non-custodial contract):**
  `sester/signer.py` â€” `Signer` and `ResultTransport` protocols,
  `PreparingSigner` (deterministic batch â†’ raw payload), and
  `build_signed_settlement()` (batch â†’ prepare â†’ optional broadcast â†’
  ...
  prepare/broadcast failures raise `SettlementError` â€” a silent accept is
...
- **Fleet-lane dogfood example (production-ready template):**
  `examples/fleet_lane/` â€” a real internal endpoint behind
...
- **Dogfood report template:** `docs/DOGFOOD_REPORT_TEMPLATE.md` â€” monthly
...
- **Warnings-as-errors discipline:** `pyproject.toml` now turns any new
  ...
  testclient deprecations from starlette/fastapi are the only exemptions â€”
...
- **Metering metrics (step 1):** `GET /metrics` â€” Prometheus-text counters
...
- **Burst limiting (step 2):** token-bucket second gate â€” per-agent
  `burst_capacity` (default 20) + `burst_refill_per_sec` (default 10);
  independent of the daily quota; over-burst â†’ 402 `rate_limited` plus a
  ledger decision event (fail-closed audit trail).
- **Evidence webhooks (step 3):** `sester/webhooks.py` â€” charge/settlement
  ...
  (3Ã— backoff) + failure log in the ledger (`webhook_delivery` event);
...
### Added
- **Hosted facilitator MVP (S5, lane-1):** `sester/facilitator_svc/` â€”
  ...
  guard â†’ 401). Non-custodial: keys never collected. Tests 186â€“201.
- **S6 joint-acceptance contract:** `docs/S6_JOINT_ACCEPTANCE.md` â€” Tenderix
  authorize/capture/refund â†” Sester facilitator mapping, single-ledger
  dual-evidence discipline, per-nonce cumulative refund cap; 63-side tests
  196â€“201 (`tests/test_s6_joint.py`).
...
- **Payee registry (settlement):** a free-form ledger identity is never passed
  into the ABI `address` field anymore â€” `register_payee` / `derive_payee_address`
  ...
  `SettlementError: adres 40-hex deÄŸil` on non-address seller ids).
- **Tamga receiver read-compat:** sibling receiver regressed to `pugio`-only
  and rejected the frozen primary `sikke` (gate-run finding, test_74) â€”
  dual-name read restored (`sikke|pugio`, unknown â†’ RED) in the sibling AND the
  canonical copy; K0 spec Â§5 documents the receiver-side read-compat rule;
...
### Identity
- **SIKKE â†’ SESTER rename** (project history: Pugio â†’ Sikke â†’ Sester, see
  the unify-log (identity history)): package `sikke` â†’ `sester`, `SikkeMeter` â†’ `SesterMeter`,
  `X-Sikke-*` â†’ `X-Sester-*` headers, `exact-sikke` â†’ `exact-sester` scheme,
  `PUGIO_*` env vars â†’ `SESTER_*`. **Frozen wire fields preserved** for
  ...
  `scripts/pugio_watch_receiver.py`) â€” unchanged until v2. A fail-loud
...
- **UCP adapter (web-monetization):** `issue_ucp_checkout()` / `verify_ucp_checkout()`
  â€” the fourth protocol joins the K0 core (`UCP-Checkout <b64>` envelope,
  ...
  `ucp_merchant` production flags (tests 159â€“166). the fourth-protocol survey watchlist
  item â†’ code.
- **On-chain settlement batches:** `sester/settlement.py` â€” pure-stdlib
  ...
  (replay-on-other-contract closed); tx-signing deliberately out of scope â€”
  non-custodial (tests 174â€“181).
- **Minor-unit counter column:** `amount_minor` helper column on both
  ...
  idempotent) â€” middleware quota decisions are now pure-integer end to end
  (tests 167â€“173). K0 frozen-canonical rule intact: the column never enters
  chain hashes; legacy rows stay dual-read.
- **Embedded mirror receivers for the frozen bridge contract:**
  `bridge_receivers/` â€” pure-stdlib, copy-ready Tamga/Veridict receivers
...
- **S5 milestone design:** `docs/S5_FACILITATOR_MILESTONE.md` â€” hosted
  facilitator MVP (monetization lane 1) targeting v0.5.0, with acceptance
  scenarios S5.aâ€“S5.e (code landed the same day â€” see [Unreleased]).
- Publication dry-run for 0.4.0 (twine PASSED) and private GitHub repo setup
  (`goun7/pugio-meter` â€” rename to `goun7/sester` at publish; cross-repo CI
  variables gated until siblings publish).
- **Publication gates as code:** `scripts/publish_gate.sh` (one-shot local gate:
  cleanup + full suite Ã—2 incl. Postgres + S1/S2 + brand PNGs + build/twine +
  live-E2E; `PUSH=1` / `REPO_RENAME=1` for remote steps) and
  `scripts/live_gate.sh` (live uvicorn E2E T0â€“T10: challenge, HMAC happy-path
  + replay, broken-HMAC/EVM â†’ 402-not-500, UCP seller envelope, quota,
...
- **EVM attribution (demo policy-guard):** `_agent_of` now verifies the
  ...
  agent string â€” previously it verified with an empty resource (which can
...
### Added
- **ACP seller-side sessions:** `issue_acp_session()` â€” the seller endpoint
  ...
  (tests 146â€“152).
- **SQLiteâ†’Postgres migration tool:** `sester/migrate_pg.py` â€” hash-preserving
  ...
  primitives on both backends (tests 153â€“158).
- **the fourth-protocol survey:** fourth player UCP (Google+Shopify, `/.well-known/ucp`,
  ...
  10 Jun 2026) â€” adapter thesis confirmed a fourth time; priorities unchanged,
  UCP on watchlist (Â§5).
- release-gate checklist (maintainers' internal record) with completed
  dry-run (twine PASSED, clean-venv smoke, zero forced deps).

### Fixed
- **Security:** ACP/AP2 JWS body-binding generalized â€” the binding now covers
...
- Escalation queue DB: WAL + busy_timeout (matches ledger discipline); demo
  DBs overridable via env for test isolation (conftest) â€” no more lock
...
- **AP2 JWS verification (RFC 7515):** mandate signatures are now cryptographically
  verified â€” HS256 (stdlib) and ES256 (`[jws]` extra, `cryptography`); opt-in
  `key_resolver` (production requires it), `expected_body` binding kills
  scope-scraping (tampered body + valid signature â†’ 402), alg-allowlist
  (`none` fail-closed), `sign_mandate_jws` producer helper (tests 108â€“117).
- **Signed policy + 24h relaxation gate (S3):** `sester/policy_signed.py` â€”
  JWS-sealed policy envelopes; tightening applies immediately, loosening
  waits `signed_at + 86400` (SPEND_POLICY Â§3); pending relaxation never leaks
  into decisions; corruption keeps last-good state, never loads (tests 118â€“128).
- **Approval panel UI:** `/approvals` brand-styled page (Sester-coin mark,
  ...
  `/escalations` exempt from metering (tests 129â€“135).
- **81-side native bundle reader (S4 closure):** Tamga repo's
  `tamga_pugio_ingest.py` â€” pure-stdlib K0 verification + deterministic
  ...
  (tests 136â€“140).
- **Postgres backend:** `sester/pg_ledger.py` â€” identical `Ledger` interface on
  the shared hash-chain core (`canonical_line`/`seal` extracted backend-neutral);
  same secret+events+timestamps â†’ identical chains across SQLite/PG; PG bundle
  passes the external pure-stdlib verifier; `ON CONFLICT` atomic `claim_nonce`;
  `[pg]` extra; parity suite with deterministic clock (tests 141â€“145).
...
- **Protocol adapters (K4):** AP2 mandate (`AP2-Mandate <b64>`) and ACP
  ...
  replay/quotum machinery â€” one wallet, one counter across protocols
  (`sester/adapters.py`, tests 82â€“95).
- **Escalation queue:** SPEND_POLICY `then: escalate` is now a real
  human-in-the-loop flow â€” park (402 `escalation_required:<id>`), approve/deny
...
- `watchfeed`/receiver invariant pinned in K0 spec Â§6:
  `bundle_event_count â‰¥ close.entries` (mixed bundles contain receipts that
  the decision feed omits).

## [0.1.0] â€” 2026-09-12

### Added
- Pure-ASGI x402-style metering middleware: 402 challenge â†’ payment â†’
  receipt headers; HMAC (`pugio0`) and EVM (`Sikke-EVM` â†’ today's
...
- x402 `exact` (EIP-3009/EIP-712) envelope support with facilitator
  verifyâ†’settle lifecycle and `settlement` ledger events; unknown/error â†’
  fail-closed 402.
- Persistent nonce table (`seen_nonces`) â€” replay protection survives
