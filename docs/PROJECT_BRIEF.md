> Public copy of an internal planning document. Internal workspaces are
> referenced as `<workspace>`; canonical history stays with the maintainers.

# 63 — SESTER (Agent Commerce Stack · Agent Commerce Stack)

> Aile: `algoat` (F1 disiplini + platform hattı) · Durum: **KÂĞIT → B-PROTOTİP** · Açılış: 2026-09-11 · **Kimlik: SESTER kesin (2026-09-13; dizisi Pugio → Sikke → Sester — identity-history record)**
...
başarısızlığı (protocol survey_TABLOSU §4 maddeleri) → SDK uygulaması durur.
...
1. ~~Bu dosyayı PRD'ye büyüt: katman A kural motoru şeması (DSL taslağı).~~ ✅ PRD v1.1 + SPEND_POLICY_V0
2. ~~x402 + AP2 spec fark tablosu çıkar~~ ✅ protocol survey_TABLOSU v2 (ACP dahil üçlü)
3. ~~B Kattı prototipi için ayrı karar kaydı aç.~~ → **SESTER-B v0 kodlandı** (``docs/adr/0003``, `sester/`)
...
`pugio_evidence_bundle`, `source:sikke` — identity-history record). Logo: `brand/sester-mark.svg`
...
- **P8 1-yaz-10-test:** kural-DSL'e her kural için test-vektör şartı — imzasız kural canlıya çıkmaz (SPEND_POLICY_V0 test maddeleriyle hizalı).
...
## 🌐 TUR-3 (2026-09-12, web-taraması — kanıt: the research memo (2026-09-12))
...
- **Üçüncü oyuncu ACP:** Stripe+OpenAI Agentic Commerce Protocol (Eyl 2025) —
  Instant Checkout canlı (Şub 2026). protocol survey_TABLOSU v2 üçlü-tabloya büyüdü;
...
- **Dürüst-küre kuralı:** x402 205M+ işlem ama gerçek-hacim ~$28K/gün,
  ...
  **gerçek-hacim + aktif-demo**; `docs/adr/0001` §2'ye hacim-taban pivot-tetikleyicisi
  eklendi.
...
- **Mevzuat-nerdi notu:** "28.08.2026 ödeme-yasağı" çıkışı yanlış-etiket
  ...
  hukuk-görüşü önceliği yükseltildi (`docs/adr/0001` §11).