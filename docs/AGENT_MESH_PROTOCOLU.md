...
# mesh PROTOKOLÜ — kardeş-proje ajan-koordinasyonu (v1, 2026-09-13)

> Partners: four sibling agent-projects (names intentionally omitted from the public copy)
> Amaç: ajanlar arası **mesaj-taşıyıcılığı kaldırmak** — koordinasyon iki
> mekanizmayla otomatikleşir: (1) **mesh ledger** (ortak kayıt-defteri,
...
```bash
bash <workspace>/projects/workspace/sester/scripts/install_hooks.sh
```
...
| `post-commit` | commit-bilgisini mesh ledger'ne ekler | ASLA bloklamaz |
...
|---|---|---|---|
| sester | `guard_sester.py` | sürüm-senkronu (pyproject==`__version__`), DONUK `source:"sikke"` üretimi; gözlem: kardeş K1-alıcısı çift-ad-okuma | `identity-history record.md`, K0 §5 |
| escrow-partner (64) | `guard_tenderix.py` | bridge_version=2: `RAILS` kanonik `sester_*`, `RAIL_ALIASES` çift-ad-okuma, `LIVE_DISABLED_UNTIL_PSP_CERT` | `63_SESTER_ALINAN_NOT.md` |
...
driftini yalnız WARN'lar** + mesh ledger'ne drift-satırı düşer (karşı-repo
...
3. Manuel olay (karar, teyit, uyarı, nota) → mesh ledger §Manuel-günlüğe
   tek-satır (Tarih | Kim | Olay).
4. Her ajan oturuma **READ-FIRST** ile başlar: mesh ledger son-20-satır.

## 4) mesh ledger yazım-kuralı (post-commit)
...
1. Kısa spec-notu (ne, neden, hangi kaynak-belgeyle) → mesh ledger §Manuel +
   karşı-tarafa ilet;
