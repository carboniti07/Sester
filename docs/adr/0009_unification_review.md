...
> `source: "sikke"`, alıcı-dosya adları — bu tablodaki köprüler etkilenmez (identity-history record).
> Yöntem: iki deponun README/ARCHITECTURE/pyproject/commit-geçmişi yerinde okundu
> (yalnız-okuma); karar kâğıt-bağı mimarisi (KAGIT/`docs/adr/0001`) üzerinden verildi.
...
| **K4 · Tamga receipt ↔ SESTER charge** | tamga↔63 | Aynı ajanın work-receipt'leri ile charge-receipt'leri tek evidence-stream'de (`evidence_ref`, `docs/adr/0001` §6 veri-modeli) | ileride |
