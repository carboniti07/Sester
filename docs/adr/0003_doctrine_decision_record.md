# `docs/adr/0003` — SESTER B-katmanı (AgentMeter) Prototip Kararı

> Tarih: 2026-09-12 · Durum: **v0.4.0 kodlandı + SIKKE→SESTER kimlik-göçü (2026-09-13)** · Üst: `docs/adr/0007` sürüm-tablosu
...
> (test_148'in yakaladığı ACP-kazıma boşluğu kapandı); protocol survey v3 (dördüncü
...
> test_159–166, protocol survey §5 izleme-kararı koda geçti); on-chain settlement-
...
> `identity-history record`). GitHub: **goun7/sester** (özel; eski-ad pugio-meter);
> para-kazanma senaryosu `docs/MONETIZATION.md` (5 gelir-hatı, adoption-önce).
> **escrow-partner (64) hizalaması (2026-09-13):** hat-geneli ad-haritası teyit-edildi
...
> kabul sözleşmesi `docs/S6_JOINT_ACCEPTANCE.md` (escrow-partner (64) ile). Koşu-kanıtı
...
63-B, `sester/` Python paketi olarak v0'a alındı (kimlik-göçü: identity-history record). Amaç: 2-3 OSS vitrin-kurulumuna
...
| Politika | `policy.py` | SPEND_POLICY v0 alt-kümesi: ilk-eşleşen, fail-closed, DenyAll | SPEND_POLICY_V0 §2 semantiği; bozuk dosya → tüm harcama durur (test_09–22) |
...
- [x] **S1 (MILESTONE_PLAN §7):** günlük-$50-cap + saat-aralığı dogfood → `scripts/s1_dogfood.py` KABUL
- [x] **S4 (MILESTONE_PLAN §7, ilk-adım):** kanıt-bundle pür-sha256 harici-doğrulanır (test_49 + dogrula.py) — 81-MERGEN hattına hazır
