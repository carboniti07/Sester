# MILESTONE PLAN — SESTER (Agent Commerce Stack) (2026-09-11, kimlik-göçü 2026-09-13: identity-history record)

> Mükemmelliyet V2 katmanı. V1: ``docs/adr/0002`` + `docs/adr/0007` + `protocol-survey table`
> (x402↔AP2 adaptör-kararları) + `SPEND_POLICY_V0.md` (fail-closed cüzdan-politikası)
...
> tehdit-modeli ve pitch-verileri `the research memo (2026-09-12)` ile tarihli
> kaynaklandı; protocol survey v2 üçlü-tabloya (x402↔AP2↔ACP) büyüdü.
...
| Ajan-ödeme-startup'ları | Skyfire (KYA-platform, günc. 8 Eyl 2026), Payman (Visa-yatırımlı), Nekuda (AmEx-seedli) | kimlik (KYA) + yetki + ödeyici tek-platforma yakınsıyor | non-custodial + protokol-tarafsız politika-katmanı (SPEND_POLICY) + kanıt-zinciri — yakınsama bizim ekseni doğrular |
...
**Hendek:** (a) protocol survey adaptör-decisyonları (ChargeIntent/ChargeReceipt soyutlama),
(b) SPEND_POLICY fail-closed politika (kurumsal-satışta şart), (c) 81-güvence ile
...
| Tehdit | Vektör | Önlem |
|---|---|---|
| Anahtar-tehlikeye | ajan-cüzdan-key sızması | non-custodial + fail-closed + 24s gevşetme-gecikmesi (SPEND_POLICY_V0) |
| Protokol-kayması | x402/AP2/ACP-üç-taraflı değişim | adaptör-soyutlama + protocol survey v2-takip (çeyrekte: LF + FIDO + ACP-repo kanalları) |
...
| ACP-kapanıklığı | checkout-akışının platform-içi yetkisi | satıcı-sırası metering+kanıt odaklı kalır; perakende-pivotu izlemede (protocol survey v2 §3) |
...
`wallet_policy` (SPEND_POLICY-yaml) · `charge_intent` (ChargeIntent) ·
...
- **3.ay:** Meter-paketi + protocol survey v3 (çeyreklik-protokol-takibi: LF x402
...
tek-kaynak) → politika-katmanı güven-hikâyesinin kanıtı (SPEND_POLICY §5).
...
- x402/AP2-galip-belirsizliği çeyrekte → protocol survey v2 + adaptör-haritası-güncelleme.
