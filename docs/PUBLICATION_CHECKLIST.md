# SESTER — Yayın-Kontrol Listesi (sester 0.6.0)

> Amaç: gerçek yayını (GitHub + PyPI) tek oturumda hatasız yapabilmek.
> Aşağıdaki her madde ya ✅ tamamlandı ya da ☐ kullanıcı-eylemi.
> Tarih: 2026-09-14 · Kimlik: **SESTER** · Sürüm: **0.6.0**
> Durum: **PyPI CANLI (0.5.0) + v0.6.0 artefaktları inşa-yeşili** —
> kalan: 0.6.0 upload + release + public-geçiş dizisi

## 1) Artefakt-hazırlığı — ✅ TAMAM (0.6.0 inşa-adımıyla)
...
- [x] PyPI hesabı + 2FA + token (.pypirc yerelde) — 2026-09-14
- [x] **`twine upload dist/sester-0.5.0*` — CANLI (2026-09-14):**
      https://pypi.org/project/sester/0.5.0/ — preupload_check TAMAM ile upload
- [x] Yayın-sonrası teyit: temiz-venv `pip install sester` → 0.5.0 import-OK;
      `[evm]` extra kuruldu; PyPI-JSON: license Apache-2.0, summary doğru
- [x] About homepage → https://pypi.org/project/sester/ (gh ile)
- [x] Sürüm-sonrası: `__init__` + `pyproject` → **0.6.0.dev0** (senkron)
...
- Donuk kablo-alanları (pugio0, pugio_bundle_version, pugio_evidence_bundle,
  source:sikke) **v2'ye kadar değişmez** (unify-log): kardeş-repo alıcıları
...
- Kaynak-yüzeyi (2026-09-13): kablo/README/CHANGELOG/SEED GitHub'ta sester-pure
  (repo-adı dışında; önceki-kimlik referansları bilinçli-kalıcı — unify-log
