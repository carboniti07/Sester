#!/usr/bin/env python3
"""SESTER pre-commit guard — contract checks (coordination doc lives outside the public tree).
...
Gözlem (WARN; bloklamaz — kendi-taraf-kusuru değildir):
  3) kardeş Tamga-alıcısı çift-ad-okuyor mu (sikke|pugio)? drift-row goes to the optional mesh ledger
...
LEDGER = Path(os.environ.get("SESTER_MESH_LEDGER", ""))  # optional; empty on public forks
SIBLING_K1 = Path(
...
def check_frozen_fields() -> int:
    ...
    if '"source": "sikke"' not in src:
        return red('DONUK kablo-alanı kayboldu: bridges.py `source: "sikke"` '
                   "(identity history — producer always emits the frozen field until protocol v2)")
...
def note_ledger(line: str) -> None:
    try:
        with LEDGER.open("a", encoding="utf-8") as fh:
            fh.write(f"| {time.strftime('%Y-%m-%d %H:%M')} | sester-guard "
                     f"| — | {line} |\n")
    except OSError:
...
def observe_sibling_k1() -> int:
    ...
    print("\033[1;33m[SESTER-guard] WARN: kardeş K1-alıcısı driftli "
          "(sikke-RED) — logged to the optional mesh ledger; commit not blocked "
    ...
    for check in (check_version_sync, check_frozen_fields, observe_sibling_k1):
        ...
        if rc:
            print("\033[1;31mcommit BLOKLANDI — --no-verify KULLANMAYIN "
                  "(see the coordination protocol's drift playbook)\033[0m")
            return rc
    return 0
