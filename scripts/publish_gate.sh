...
step "3) Kabul-kapıları — S1 dogfood + S2 dört-protokol parite + v0.6.0 özellik-testleri"
.venv/bin/python scripts/s1_dogfood.py
.venv/bin/python scripts/s2_protocol_parity.py
.venv/bin/python -m pytest tests/test_v060_metrics_rate.py tests/test_webhooks.py -q

step "4) Marka-varlık PNG'leri (OG 1280×640 + avatar 512×512)"
.venv/bin/python scripts/make_brand_assets.py

step "5) 0.6.0 artefaktları — build + twine check"
...
git add -A
git commit -m "SESTER v0.6.0 — roadmap 1-3 (metrics, burst-limit, evidence webhooks) + public-surface hygiene" \
...
printf '\n\033[1;32mYAYIN-KAPISI TAMAM — v0.6.0 yayında.\033[0m\n'
