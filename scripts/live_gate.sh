#!/usr/bin/env bash
# SESTER canlı-E2E kapısı — gerçek uvicorn-sürecinde tüm akışlar (v0.6.0).
#
...
echo "$HZ" | grep -q '"version": *"0.6.0"' && ok "healthz version=0.6.0" || red "healthz sürüm: $HZ"
...
printf '\n'
if [[ "$FAIL" -eq 0 ]]; then
  printf '\033[1;32mCANLI-KAPI TAMAM — %s doğrulama geçti (v0.6.0 canlı akışlar).\033[0m\n' "$PASS"
  exit 0
else
