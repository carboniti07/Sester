...
Toplam olay: {total_events} · Harcama (bugün, tüm-ajanlar): <span class="gold">{spent_today_total:.4f}</span> ·
Ücretli-işlem (bugün): <span class="gold">{charges_today}</span> · <code>GET /metrics</code> makine-okur sayaçlar.</p>
</div></body></html>"""
...
def render_panel(ledger: Any, quota: float) -> str:
    ...
    # v0.6.0 özet-satırı: bugünün toplamları (aynı ledger-döngüsünden; sıfır-ek-sorgu değil ama ucuz)
    events_all = ledger.export_events()
    day = time.strftime("%Y-%m-%d")
    charges_today = sum(
        1 for e in events_all
        if e["event_type"] == "charge_receipt" and time.strftime("%Y-%m-%d", time.localtime(e["ts"])) == day)
    spent_today_total = sum(
        (e.get("amount_minor") or 0) for e in events_all
        if e["event_type"] == "charge_receipt" and time.strftime("%Y-%m-%d", time.localtime(e["ts"])) == day) / 1_000_000
    return PAGE.format(
        mark=MARK_SVG,
        ts=time.strftime("%Y-%m-%d %H:%M:%S"),
        favicon=FAVICON_HREF,
        spent_today_total=spent_today_total,
