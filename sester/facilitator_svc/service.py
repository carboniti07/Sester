...
class FacilitatorService:
    ...
    def __init__(self, ledger: Ledger, *, secret: str = "facilitator-secret",
        ...
        # v0.6.0 adım-1 (servis-yüzeyi): Prometheus-text satıcı-metrikleri
        self.metrics = {"verify_ok": 0, "verify_rejected": 0, "settled": 0,
                        "settle_rejected": 0, "refunded": 0, "refund_rejected": 0}
    ...
    def _metrics_text(self) -> str:
        ...
        lines = [f"# TYPE sester_facilitator_{k} counter"
                 f"\nsester_facilitator_{k} {v}" for k, v in m.items()]
        lines.append("# TYPE sester_facilitator_chain_valid gauge")
        lines.append(f"sester_facilitator_chain_valid {chain}")
        return "\n".join(lines) + "\n"
